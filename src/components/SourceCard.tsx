      ...
      <div>
        ...
        <div className="flex items-center justify-center gap-2 my-2">
          <button
            ...
            className="px-3 py-1 rounded-lg text-xs font-bold bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 transition-all cursor-pointer shadow-2xs"
          >
            -1 hr
          </button>
          <button
        ...
        <div className="space-y-3 mt-3">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full px-3.5 py-2 rounded-xl bg-white/70 dark:bg-zinc-800/80 border border-black/10 dark:border-white/15 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-2xs transition-all cursor-pointer backdrop-blur-md"
            />
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 relative">
                <select
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-white/70 dark:bg-zinc-800/80 border border-black/10 dark:border-white/15 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-2xs transition-all cursor-pointer backdrop-blur-md appearance-none text-center"
                >
                  {hoursOptions.map((h) => (
                    <option key={h} value={h} className="text-zinc-900 bg-white dark:bg-zinc-800 dark:text-zinc-100">
                      {h}
                    </option>
                  ))}
                </select>
              ...
              <div className="flex-1 relative">
                ...
                >
                  {minutesOptions.map((m) => (
                    <option key={m} value={m} className="text-zinc-900 bg-white dark:bg-zinc-800 dark:text-zinc-100">
                      {m}
                    </option>
                  ))}
                </select>
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-black text-sm transition-all cursor-pointer shadow-md active:scale-95 border border-blue-400/30"
              >
                {ampm}
              </button>
            </div>
      ...
      <div className="mt-5 pt-3 border-t border-black/5 dark:border-white/10 text-center text-xs font-semibold opacity-80 flex items-center justify-center gap-1.5">
        {isDay ? (
          <>
            ...
            <span>Sunset at <strong>{formattedTime.sunInfo.sunsetStr}</strong> (Sunrise was {formattedTime.sunInfo.sunriseStr})</span>
          </>
        ) : (
          <>
            <span className="text-indigo-400">🌅</span>
            <span>Sunrise at <strong>{formattedTime.sunInfo.sunriseStr}</strong> (Sunset was {formattedTime.sunInfo.sunsetStr})</span>
          </>
        )}
      </div>
    </div>
